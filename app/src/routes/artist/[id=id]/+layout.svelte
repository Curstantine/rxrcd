<script>
	...
	import { tabs, extort_tab_state, style_tab, style_tab_bar } from "./page.hooks";
	...
	const { artist: artistPromise } = data;
</script>
...
<div class="flex flex-col container">
	...
	<div use:style_tab_bar style="--un-bg-color-opacity: 1;" class="tab_bar">
		{#each tabs as { id, label, rel_type }}
			<a {id} class="tab" class:active={$active_tab.id === id} href="/artist/{}/{rel_type}">{label}</a>
		{/each}
